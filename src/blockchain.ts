...
const BLOCKCHAIN_FILE_TMP = 'data/blockchain.json.tmp';
...
const initGenesisBlock = async (): Promise<void> => {
...
const saveBlockchain = async (force: boolean = false) => {
    const currentHeight = getLatestBlock() ? getLatestBlock().index : 0;

    // During sync, only save every N blocks to avoid I/O overload
    if (!force && isSyncing && (currentHeight - lastSaveHeight < BLOCKS_PER_SAVE_DURING_SYNC)) {
        return;
    }
    ...
    return new Promise<void>((resolve, reject) => {
        try {
            ...
            const writeStream = fs.createWriteStream(BLOCKCHAIN_FILE_TMP);

            writeStream.on('error', (err) => {
                console.log('Error writing blockchain to tmp file: ' + err.message);
                reject(err);
            });

            writeStream.on('finish', () => {
                try {
                    if (fs.existsSync(BLOCKCHAIN_FILE_TMP)) {
                        fs.renameSync(BLOCKCHAIN_FILE_TMP, BLOCKCHAIN_FILE);
                        lastSaveHeight = currentHeight;
                        // console.log(`Blockchain saved at height ${currentHeight} (RAM-safe streaming)`);
                    }
                    resolve();
                } catch (err) {
                    console.log('Error renaming blockchain tmp file: ' + (err as any).message);
                    reject(err);
                }
            });
            ...
            console.log('Error in saveBlockchain streaming: ' + (e as any).message);
            reject(e);
        }
    });
};

const existsSync = (path: string) => fs.existsSync(path);
const readFileSync = (path: string, encoding: any) => fs.readFileSync(path, encoding);
const mkdirSync = (path: string) => fs.mkdirSync(path);
