...
const initGenesisBlock = async () => {
    ...
    try {
        ...
        setUnspentTxOuts((0, transaction_1.processTransactions)(blockchain[0].data, [], 0));
...
const setIsSyncing = (status) => {
    if (isSyncing !== status) {
        console.log(`Sync status changed: ${isSyncing} -> ${status}`);
    }
    isSyncing = status;
};
exports.setIsSyncing = setIsSyncing;
